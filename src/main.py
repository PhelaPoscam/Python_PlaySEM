...
async def main():
    ...
    try:
        ...
        logging.info(
            f"Successfully loaded device configuration from '{args.devices_config}'."
        )
    ...
    for interface_config in devices_config.get("connectivityInterfaces", []):
        try:
            logging.info(
                f"Creating driver for interface: {interface_config.get('name')}"
            )
            ...
            logging.error(
                f"Failed to create driver for interface {interface_config.get('name')}: {e}"
            )

    if not drivers:
        logging.warning(
            "No drivers were created. Please check your configuration file."
        )
        return

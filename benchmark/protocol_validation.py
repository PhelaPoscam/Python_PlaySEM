...
This is a standalone validation and benchmark script and NOT a pytest-compatible test file.
It is intended for manual execution or specific performance measurement jobs.
"""
...
class MockEffectDispatcher:
    ...
    async def dispatch_effect(
        self, device_id: str, effect_data: Dict[str, Any]
    ):
...
class ProtocolBenchmark:
    ...
    async def test_websocket_handler(self) -> Dict:
        ...
        from tools.test_server.handlers.websocket_handler import (
            WebSocketConfig,
        )
    ...
    def print_report(self, results: Dict[str, Dict]):
        ...
        for protocol, result in results.items():
            ...
            if result.get("passed"):
                ...
                print(
                    f"  Range: {result['min_latency_ms']}ms - "
        ...
        if all(r.get("passed") for r in results.values()):
            latencies = [
                (r["protocol"], r["avg_latency_ms"]) for r in results.values()
            ]

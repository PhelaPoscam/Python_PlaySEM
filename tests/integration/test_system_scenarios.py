...
async def test_mqtt_to_vibration_e2e(playsem_system):
    ...
    try:
        ...
        for entry in system.mock_driver.command_history:
            if entry["device_id"] == "vibration_device" and entry["command"] == "set_intensity":
                if entry["params"].get("intensity") == 85:
                    found = True
                    break
        
        assert found, f"Vibration command not found in journal. History: {system.mock_driver.command_history}"
...
async def test_cross_protocol_consistency(playsem_system):
    ...
    # 1. Direct Dispatch
    effect = EffectMetadata(effect_type="light", intensity=50, parameters={"r": 255, "g": 0, "b": 0})
    ...
    client.loop_start()
    try:
        client.publish("effects/light_device", json.dumps({
            "effect_type": "light", 
            "intensity": 100, 

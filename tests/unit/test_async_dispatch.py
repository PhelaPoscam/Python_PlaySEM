...
async def test_device_manager_async_send_command():
    ...
    success = await manager.async_send_command("dev1", "set_intensity", {"intensity": 100})
    
    assert success is True
    assert len(driver.commands_received) == 1
    assert driver.commands_received[0] == ("dev1", "set_intensity", {"intensity": 100})
...
async def test_effect_dispatcher_async_dispatch():
    ...
    assert len(driver.commands_received) == 1
    assert driver.commands_received[0] == ("light_device", "set_brightness", {"value": 50})

...
class ConnectedDevice:
    ...
    def __init__(self, device_id: str, device_name: str, device_type: str, 
                 capabilities: List[str], protocols: List[str], connection_mode: str = "direct",
...
class ControlPanelServer:
    ...
    class _DummyDispatcher:
        ...
        def dispatch_effect_metadata(self, effect):
            print(f"[DISPATCH] Effect received by embedded server: {getattr(effect, 'effect_type', 'unknown')}")
    ...
    async def _ensure_protocol_servers(self, protocols: List[str]):
        ...
        if "mqtt" in protocols and not self.mqtt_ready:
            try:
                self.mqtt_server = MQTTServer(dispatcher=self._DummyDispatcher(), host="0.0.0.0", port=1883)
                # MQTTServer.start() is sync (spawns thread)
                await asyncio.to_thread(self.mqtt_server.start)
                self.mqtt_ready = True
                print("[BOOT] Embedded MQTT broker started on 0.0.0.0:1883")
            except Exception as e:
                print(f"[BOOT] Failed to start embedded MQTT broker: {e}")
                self.mqtt_ready = True  # Mark ready even on failure, server can continue
        ...
        if "coap" in protocols and not self.coap_ready:
            try:
                ...
                print("[BOOT] Embedded CoAP server started on coap://0.0.0.0:5683/effects")
            except Exception as e:
        ...
        if "upnp" in protocols and not self.upnp_ready:
            try:
                ...
                print(f"[BOOT] Embedded UPnP server started at http://{self.upnp_server.http_host}:{self.upnp_server.http_port}")
            except Exception as e:
    ...
    def _setup_routes(self):
        ...
        async def route_effect(protocol: str, effect_msg: dict, endpoint: Optional[Dict[str, Any]]) -> bool:
            ...
            if protocol == "mqtt":
                ...
                cfg = MQTTConfig(**{**self.mqtt_handler.config.__dict__, **endpoint}) if endpoint else self.mqtt_handler.config
                handler = MQTTHandler(config=cfg)
                return await handler.send(effect_msg)
            if protocol == "http":
                cfg = HTTPConfig(**{**self.http_handler.config.__dict__, **endpoint}) if endpoint else self.http_handler.config
                handler = HTTPHandler(config=cfg)
                return await handler.send(effect_msg)
            if protocol == "coap":
                cfg = CoAPConfig(**{**self.coap_handler.config.__dict__, **endpoint}) if endpoint else self.coap_handler.config
            ...
            if protocol == "serial":
                ...
                try:
                    ...
                    return False
            if protocol == "upnp":
                cfg = UPnPConfig(**{**self.upnp_handler.config.__dict__, **endpoint}) if endpoint else self.upnp_handler.config
        ...
        @self.app.get("/")
        async def root():
            return HTMLResponse("<h1>PlaySEM Test Server</h1><p>Running on port 8090</p>")
            ...
            try:
                ...
                while True:
                    ...
                    if msg_type == "scan_serial":
                        ...
                        devices = [
                            {"port": "COM3", "name": "Mock Light 1", "type": "light"},
                            {"port": "COM4", "name": "Mock Device 2", "type": "vibrator"},
                            {"port": "COM5", "name": "Mock Receiver", "type": "receiver"},
                        ]
                        await websocket.send_json({
                        ...
                        print(f"[REGISTER] Device: {device_name} ({device_id}) Type: {device_type} Mode: {connection_mode}")
                        
                        await self._ensure_protocol_servers(data.get("protocols", []))

                        # Build/augment protocol endpoints
                        provided_endpoints = data.get("metadata") or data.get("protocol_endpoints") or data.get("endpoints") or {}
                        endpoints = provided_endpoints if isinstance(provided_endpoints, dict) else {}

                        if isinstance(endpoints, dict):
                            if "mqtt" in data.get("protocols", []) and "mqtt" not in endpoints:
                                endpoints["mqtt"] = {
                                    "host": "localhost",
                                    "port": 1883,
                            ...
                            if "coap" in data.get("protocols", []) and "coap" not in endpoints:
                                endpoints["coap"] = {
                                    "host": "localhost",
                                    "port": 5683,
                                    "path": "effects",
                                }
                            if "http" in data.get("protocols", []) and "http" not in endpoints:
                                endpoints["http"] = {
                                    "url": "http://localhost:8090/api/effects/inbox",
                                }
                            if "upnp" in data.get("protocols", []) and "upnp" not in endpoints:
                        ...
                        print(f"[SEND_EFFECT] Attempting to send effect to device: {device_id}")
                        print(f"[SEND_EFFECT] Available devices: {list(self.devices.keys())}")
                        ...
                        print(f"[SEND_EFFECT] Device found: {device.device_name}, has websocket: {device.websocket is not None}")
                        ...
                        endpoints = meta.get("protocol_endpoints") or meta.get("endpoints") or meta
                        if isinstance(endpoints, dict):
                        ...
                        if endpoint:
                            ...
                            sent = await route_effect(chosen_protocol, effect_msg, endpoint_clean)
                            if sent:
                                print(f"[SEND_EFFECT] Sent via {chosen_protocol} endpoint: {endpoint}")

                        # Fallback to device websocket if not already sent
                        if not sent and device.websocket:
                            try:
                                print(f"[SEND_EFFECT] Sending effect to device connection: {effect_msg}")
                                await device.websocket.send_json(effect_msg)
                                sent = True
                                print(f"[SEND_EFFECT] Effect sent successfully to device!")
                            except Exception as e:
                                print(f"[SEND_EFFECT] Failed to send to device websocket: {e}")

                        # Fallback: broadcast to all other connections (e.g., if device websocket changed)
                        if not sent:
                            print("[SEND_EFFECT] Fallback broadcast to other connections")
                            for conn in self.active_connections:
                                if conn != websocket:
                                    try:
                                        ...
                                        print(f"[SEND_EFFECT] Fallback send failed: {e}")
                        ...
                        await self.start_protocol_server(websocket, data.get("protocol"))
        ...
        async def register_device(body: dict):
            ...
            provided_endpoints = body.get("metadata") or body.get("protocol_endpoints") or body.get("endpoints") or {}
            endpoints = provided_endpoints if isinstance(provided_endpoints, dict) else {}
            if isinstance(endpoints, dict):
                if "mqtt" in body.get("protocols", []) and "mqtt" not in endpoints:
                    endpoints["mqtt"] = {"host": "localhost", "port": 1883, "topic": f"effects/{device_id}", "ws_port": 9001}
                if "coap" in body.get("protocols", []) and "coap" not in endpoints:
                    endpoints["coap"] = {"host": "localhost", "port": 5683, "path": "effects"}
                if "http" in body.get("protocols", []) and "http" not in endpoints:
                    endpoints["http"] = {"url": "http://localhost:8090/api/effects/inbox"}
                if "upnp" in body.get("protocols", []) and "upnp" not in endpoints:
                    endpoints["upnp"] = {"control_url": "http://localhost:8008/control"}
            ...
            for yaml_file in sorted(config_dir.glob("device_*.yaml")):
                try:
                    ...
                    templates.append({
                        ...
                        "protocol": device_info.get("connection", {}).get("protocol", "unknown"),
                    })
                except Exception as e:
        ...
        async def load_device(body: dict):
            ...
            try:
                ...
                if not device_id:
                    raise HTTPException(status_code=400, detail="Device config missing 'id'")
                ...
                print(f"[LOADED] Device from config: {device_id} ({device.device_name})")
                ...
                raise HTTPException(status_code=500, detail=f"Failed to load device: {e}")
        ...
        async def super_controller():
            ui_file = Path(__file__).parent.parent / "ui_demos" / "super_controller.html"
            if ui_file.exists():
                return FileResponse(str(ui_file), media_type="text/html")
            return HTMLResponse("<h1>super_controller.html not found</h1>", status_code=404)

        @self.app.get("/mobile_device")
        async def mobile_device():
            ui_file = Path(__file__).parent.parent / "ui_demos" / "mobile_device.html"
            if ui_file.exists():
                return FileResponse(str(ui_file), media_type="text/html")
            return HTMLResponse("<h1>mobile_device.html not found</h1>", status_code=404)

        @self.app.get("/super_receiver")
        async def super_receiver():
            ui_file = Path(__file__).parent.parent / "ui_demos" / "super_receiver.html"
            if ui_file.exists():
                return FileResponse(str(ui_file), media_type="text/html")
            return HTMLResponse("<h1>super_receiver.html not found</h1>", status_code=404)
    ...
    def _device_to_dict(self, device):
        ...
        return {
            ...
            "protocol_endpoints": getattr(device, "metadata", {}).get("protocol_endpoints", {}),
        }
    ...
    async def start_protocol_server(self, websocket: WebSocket, protocol: str):
        ...
        if websocket is not None:
            payload = {"type": "protocol_status", "protocol": protocol, "running": True}
            try:
    ...
    async def stop_protocol_server(self, websocket: WebSocket, protocol: str):
        ...
        if websocket is not None:
            payload = {"type": "protocol_status", "protocol": protocol, "running": False}
            try:
                await websocket.send_json(payload)
            except Exception as e:
                print(f"[PROTO_STATUS] send_json failed: {e}")
    ...
    async def send_effect_protocol(self, websocket: WebSocket, protocol: str, effect: dict):
    ...
    async def send_effect(self, websocket: WebSocket, device_id: str, effect: dict):
        ...
        if connection_mode == "isolated":
            ...
            if device_id in self.web_clients:
                ...
                target_proto = "websocket" if "websocket" in protocols else (protocols[0] if protocols else "websocket")
        ...
        if target_proto == "websocket" and device_id in self.web_clients:
            try:
                await self.web_clients[device_id].send_json({"type": "effect", **effect})
                return True
            except Exception as e:
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("\nUI Demos:")
    print("  http://localhost:8090/super_controller")
    print("  http://localhost:8090/mobile_device")
    print("  http://localhost:8090/super_receiver")
    print("\n" + "=" * 70 + "\n")
